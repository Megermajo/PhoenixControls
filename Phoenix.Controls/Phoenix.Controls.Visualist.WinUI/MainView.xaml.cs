...
public sealed partial class MainView : UserControl, IPillarShellHost, ICanExecuteUndoRedo
{
    ...
    private async void OnCompositorReference(object? sender, EventArgs e)
    {
        try
        {
            if (WidgetNodeRegistry.Templates.Count == 0)
                NodeTemplates.RegisterAll();

            var sb = new StringBuilder();
            sb.AppendLine($"WidgetNodeRegistry — {WidgetNodeRegistry.Templates.Count} templates");
            sb.AppendLine($"Allowed categories : {string.Join(", ", WidgetNodeRegistry.AllowedCategories)}");
            sb.AppendLine($"Forbidden categories: {string.Join(", ", WidgetNodeRegistry.ForbiddenCategories)}");
            sb.AppendLine();

            var grouped = WidgetNodeRegistry.Templates.Values
                .GroupBy(t => t.Category)
                .OrderBy(g => g.Key);
            foreach (var grp in grouped)
            {
                sb.AppendLine($"── {grp.Key} ({grp.Count()})");
                foreach (var t in grp.OrderBy(t => t.Title))
                {
                    int ins  = t.Inputs?.Count  ?? 0;
                    int outs = t.Outputs?.Count ?? 0;
                    sb.AppendLine($"  {t.Title.PadRight(28)} in:{ins,-2}  out:{outs,-2}");
                }
                sb.AppendLine();
            }
            ...
            {
                XamlRoot         = XamlRoot,
                Title            = "Compositor Reference",
                Content          = new ScrollViewer { Content = text, VerticalScrollBarVisibility = ScrollBarVisibility.Auto, HorizontalScrollBarVisibility = ScrollBarVisibility.Auto, MaxHeight = 540 },
