...
/// <summary>
/// Wraps a <see cref="CounterRoles"/> checkmark set as six two-way <c>bool?</c>
/// properties so a counter row can render the exact SIX checkboxes the model
...
/// schedules a debounced save through <c>_changed</c>. Mirrors LoyaltyRolesVm exactly.
/// </summary>
public sealed class CounterRolesVm : ObservableObject
{
    private readonly CounterRoles _roles;
    private readonly Action _changed;

    public CounterRolesVm(CounterRoles roles, Action changed)
    {
        _roles = roles ?? new CounterRoles();
        _changed = changed;
    }
