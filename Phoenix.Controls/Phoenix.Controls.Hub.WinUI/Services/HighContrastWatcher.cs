...
/// <summary>
/// Hub-side query/subscription for the Windows high-contrast accessibility
/// setting. <see cref="MainWindow"/> consults this on construct and on
/// <see cref="AccessibilitySettings.HighContrastChanged"/> so the coal/ember
/// chrome can collapse and the window can fall back to the system title bar —
/// Design_Orders §4.7 mandates the fallback under HC themes.
///
/// <para>
/// Detection only — no paint code. Per the per-pillar chrome rule, each
/// pillar owns its own copy of this helper (Hub has this file; Architect has
/// the mirror copy in <c>Phoenix.Controls.Architect.WinUI/Services/</c>).
/// It lives at the pillar level rather than in <c>Shared.WinUI</c> because
